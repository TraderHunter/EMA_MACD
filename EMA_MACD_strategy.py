...
def order_index(coinname, timetick):
    ...
    sellState = False
    zs_price = 0
    if EMA12[-2] < EMA25[-2] and EMA12[-1] > EMA25[-1] and MACD[-1] > 0 and red_line[-1]>yellow_line[-1] and (red_line[-1]!=0 or yellow_line[-1]!=0) and float(okex_api.get_current_price(coinname)[0]['last']) > EMA200[-1]:
        buyState = True
...
def sample_test(instId, sz, sz1):
    ...
    if okex_api.getPositions(instId)['data'][0]['pos'] == '0':
        ...
        if sellState == True:
            ...
            okex_api.dingding_alarm1(result)
        if buyState == True:
            para['coinname'] = instId
            para['timetick'] = '15m'
            para['zs_price'] = zs_price
        ...
        #    f2.close()
    #if  okex_api.getPositions(instId)['data'][0]['pos'] != '0':
    #    buyState, sellState = hand_state(instId, '15m')
    #    print(okex_api.getPositions(instId)['data'][0]) 
    #    f = open(instId+'.json', 'r')
    #    content = f.read()
    #    para = json.loads(content)
    #    if para['side'] == 'buy' and float(okex_api.get_current_price(instId)[0]['last']) > para['zy_price']:
    #        okex_api.create_order(instId, tdMode, 'sell', sz1, para['ids'])
    #    if para['side'] == 'sell' and float(okex_api.get_current_price(instId)[0]['last']) < para['zy_price']:
    #        okex_api.create_order(instId, tdMode, 'buy', sz1, para['ids'])
    #    if para['side'] == 'buy' and float(okex_api.get_current_price(instId)[0]['last'])<para['zs_price']:
    #        okex_api.closePosition(instId)
    #    if para['side'] == 'sell' and float(okex_api.get_current_price(instId)[0]['last'])>para['zs_price']:
    #        okex_api.closePosition(instId) 
    #    if para['side'] == 'buy'  and sellState == True:
    #        okex_api.closePosition(instId)
    #    if para['side'] == 'sell' and buyState == True:
    #        okex_api.closePosition(instId)
    #    f.close()
        
def trading():
    #sample_test('EOS-USDT-SWAP','60','20')
    #sample_test('ARB-USDT-SWAP','60','20')
    ...
    #sample_test('ADA-USDT-SWAP','60','20')
    #
    
    sample_test('DOGE-USDT-SWAP','60','20')
